...
- [x] Convenience `ProcessRequest` factory API — `ProcessRequest.ForStep<TStep>(step, processId?)` resolves step name from `[ProcessStep]` attribute, falls back to type name
- [x] Client registry cache refresh — `void InvalidateRegistry()` added to `IKaleidoProcessClient` and `IKaleidoQueryableClient`; delegates to `HttpClientRegistryCache<T>.Reset()`
...
- [x] Tier 2 complete
- [x] Tier 3 complete — branch `cleanup/tier3-route-options-map`
- [x] Tier 4 complete — branch `cleanup/tier4-breaking-changes`, 5 commits; deferred items documented inline
