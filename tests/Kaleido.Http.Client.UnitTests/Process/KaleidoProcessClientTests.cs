...
public sealed class KaleidoProcessClientTests
{
    ...
    public async Task GetProcessStateAsync_WhenFound_ReturnsState()
    {
        ...
        // The state endpoint does not require registry lookup G«ˆ just build the URL from options
    ...
    public async Task ExecuteStepAsync_ResolvesStepUrlAndPosts()
    {
        ...
        // MyStep type name G«ˆ "Step" suffix stripped: MyStep GÂ∆ MyStep (no suffix here, keep as-is)
        // Actually step name lookup uses type name with optional "Step" suffix stripping.
        // Our type below is named MyClientStep GÂ∆ strips to MyClient, won't match "MyStep".
    ...
    // "MyStepStep" GÂ∆ strip "Step" suffix GÂ∆ name is "MyStep", matches FakeStep.Name
